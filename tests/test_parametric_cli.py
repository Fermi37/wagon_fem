...
def test_parametric_cli_generates_tank_wagon_csv(tmp_path):
    output = tmp_path / "tank_wagon.csv"
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "wagon_fem.parametric",
            "docs/parametric_generator_v0_3_0/params.tank_wagon.example.yaml",
            "--output",
            str(output),
            "--validate",
        ],
